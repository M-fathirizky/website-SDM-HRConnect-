document.addEventListener('DOMContentLoaded', () => {
    const leaveTable = document.querySelector('.leave-table tbody');
    ...
    const openConfirmationModal = (row, actionType) => {
        ...
        confirmationMessage.textContent = `Apakah Anda yakin ingin ${actionText} pengajuan cuti ini?`;
    ...
    const openResultModal = (isSuccess) => {
        if (isSuccess) {
            ...
            resultMessage.textContent = `Pengajuan cuti telah di${action === 'approve' ? 'setujui' : 'tolak'}.`;
        } else {
